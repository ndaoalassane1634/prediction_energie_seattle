...
def plot_total_ghg_emissions_distribution(df):
    ...
    sns.histplot(df['TotalGHGEmissions'], kde=True, bins=30, color='blue', ax=ax)
    ax.set_title('Distribution des émissions totales de GES')
...
def plot_scatterplot(df):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(df.index, df['SiteEnergyUse(kBtu)'], alpha=0.5)
    ax.set_title('Distribution de la consommation énergétique')
    ax.set_xlabel('Index')
    ax.set_ylabel('Site Energy Use (kBtu)')
    st.pyplot(fig)  # ✅ Passer la figure plt
...
def plot_year_built_distribution(df):
    ...
    ax.set_title('Distribution selon l\'année de construction (groupée par intervalles de 5)')
    ax.set_xlabel('Années de construction')
    ax.set_ylabel('Nombre de bâtiments')
